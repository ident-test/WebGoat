...
#
# Supported file formats: .zip  .tar  .tar.gz  .tgz
#
# AUTH:
...
#   2. PUT  → direct S3 upload (no SCA headers)
...
# ──────────────────────────────────────────────────────────────
SCA_BASE_URL="${SCA_BASE_URL:-"http://xxxxxx:19778"}"
...
usage() {
  cat << 'EOF'
Usage: SCA_TOKEN=<token> ./upload-and-scan.sh <file>
...
Optional environment variables:
  ...
  SCAN_TYPE           sourceCode|gitlab|svn|git|docker|nexus2|bitbucket|merging (default: sourceCode)
...
# ──────────────────────────────────────────────────────────────
# Validate file extension — .zip .tar .tar.gz .tgz accepted
# ──────────────────────────────────────────────────────────────
validate_file_ext() {
  local f="$1"
  case "$f" in
    *.zip|*.tar|*.tar.gz|*.tgz) return 0 ;;
    *) die "Unsupported file type: $(basename "$f"). Accepted: .zip  .tar  .tar.gz  .tgz" ;;
  esac
}

# ──────────────────────────────────────────────────────────────
# Map file extension to MIME type so the S3 PUT Content-Type
# matches what the server signed the pre-signed URL with.
# If the SCA server signs with no Content-Type constraint, any
# value works — but when it does constrain it, mismatching
# causes a SignatureDoesNotMatch error and an empty response.
# ──────────────────────────────────────────────────────────────
get_content_type() {
  local f="$1"
  case "$f" in
...
# On failure, print the first 500 chars of the raw body.
# ──────────────────────────────────────────────────────────────
assert_success() {
  local resp="$1"
  local label="$2"
...
# ──────────────────────────────────────────────────────────────
# Authenticated curl wrapper — injects "token: " header
...
upload_parts() {
  ...
  local content_type="$4"   # must match what the server used when signing the URL

  local total_parts
  total_parts=$(echo "$upload_url_list_json" | jq 'length')
  info "Uploading ${total_parts} part(s) to S3 (Content-Type: ${content_type}) ..."
  ...
  split -b "${chunk_size_bytes}" "$input_file" "${tmp_dir}/part_"
  ...
  for part_file in "${part_files[@]}"; do
    ...
    # Content-Type must match the value the SCA server used when generating
    # the pre-signed URL signature. A mismatch causes AWS SignatureDoesNotMatch
    # and curl gets an empty/XML error response → the complete step then fails.
    curl -sf \
      -X PUT \
...
main() {
  ...
  # Accept .zip  .tar  .tar.gz  .tgz — reject anything else
  validate_file_ext "$input_file"
  ...
  file_size_bytes=$(wc -c < "$input_file" | tr -d ' \n')

  # S3 requires each part ≥ 5 MB (except the last). Enforce the floor.
  ...
  # Detect MIME type from extension so it matches the server's pre-signed signature
  local content_type
  content_type=$(get_content_type "$input_file")
  upload_parts "$input_file" "$chunk_size_bytes" "$upload_url_list_json" "$content_type"
  echo "" >&2
